...
{
    for (auto file : patchInfo.files) {
        auto extension = system::getExtension(file.filename);

        if (extension != ".vcv") {
            continue;
        }

        auto path = this->getPatchFilename(patchInfo, file);

        if (!system::exists(path)) {
            this->downloadFile(file.url, path);
        }
    }
}

void QueryCache::downloadFile(std::string url, std::string path)
{
    this->client.downloadPatch(url, path);
}
