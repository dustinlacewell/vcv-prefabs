...
EditableTextBox::EditableTextBox() : HoverableTextBox(), TextField() {
...
void EditableTextBox::onDragHover(const event::DragHover& e) {
    TextField::onDragHover(e);
}

void EditableTextBox::onHover(const event::Hover& e) {
    TextField::onHover(e);
    HoverableTextBox::onHover(e);
}

void EditableTextBox::onHoverScroll(const event::HoverScroll& e) {
...
void EditableTextBox::onDeselect(const event::Deselect& e) {
    isFocused = false;
    HoverableTextBox::text = TextField::text;
    e.consume(NULL);
}

void EditableTextBox::step() {
    TextField::step();
}
...
void EditableTextBox::draw(const DrawArgs& args) {
    ...
    if (isFocused) {
        ...
        // font face, size, alignment etc should be the same as for LedLabel after the above draw call
        ...
        nvgRect(vg,
            textOffset.x + (HoverableTextBox::box.size.x * 0.5) + (begin - 0.5f * TextField::text.size()) * char_width -
                1,
            ymargin,
