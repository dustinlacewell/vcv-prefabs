...
Prefab::Prefab()
{
    ...
    this->usedPlugins = std::map<std::string, std::map<std::string, int>>();
}
...
void Prefab::addTag(std::string tag)
{
...
{
    auto shortName = filename.substr(filename.find_last_of("/\\") + 1);
    return shortName.substr(0, shortName.find_last_of("."));
}

void Prefab::addPlugin(std::string pluginName, std::string moduleName)
{
    bool found = false;
...
std::string Prefab::missingReport()
{
...
void Prefab::fromJson(json_t* rootJ)
{
...
// implement whatever's necessary to put these Prefab structs into sets
// operators implementation begin:
